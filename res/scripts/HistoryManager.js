...
var HistoryManager = {
	...
	register: function(key, defaults, onMatch, onGenerate, regexp, options) {
		...
		this.fireEvent('onRegister', [key, data]);
		return {
			setValues: function(values) { return this.setValues(key, values); }.bind(this),
			setValue: function(index, value) { return this.setValue(key, index, value); }.bind(this),
			generate: function(values) { return this.generate(key, values); }.bind(this),
			unregister: function() { return this.unregister(key); }.bind(this)
		}
	},
	...
	setValues: function(key, values) {
		...
		if (data.values.isSimilar(values)) return this;
		data.values = values;
		this.update();
		return this;
	},
	...
	setValue: function(key, index, value) {
		var data = this.modules.get(key);
		if (data.values[index] == value) return this;
	...
	observe: function() {
		...
		if (window.ie || window.oldsafari && (this.state != null)) this.setState(state, true)
		...
		this.fireEvent('onObserverChange', [state]);
	},

	generateState: function() {
		var state = [];
		this.modules.each(function(data, key) {
			if (data.values.isSimilar(data.defaults) && data.skipDefaultMatch) return;
			state.push(data.onGenerate(data.values));
		});
		state = state.join(this.options.stateSeparator);
		return state;
	},
	...
	getState: function() {
		...
		if (window.oldsafari && history.length != this.count) {
			this.count = history.length;
		...
		if (window.ie && (!fix || this.istateOld)) {
			if (!this.iframe) {
				this.iframe = new Element('iframe', {
					'src': this.options.iframeSrc, 'styles': 'visibility: hidden;'
...
// for now the only way to check for webkit nightly/safari3
if (window.khtml && navigator.userAgent.test('AppleWebKit')) {
	var ver = navigator.userAgent.match(/AppleWebKit\/(\d*)/);
	if (ver && parseInt(ver[1]) < 420) window.oldsafari = true;
}