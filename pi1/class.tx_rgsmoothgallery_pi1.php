...
class tx_rgsmoothgallery_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	function init($conf) {
		...
		/*if (class_exists('t3lib_utility_VersionNumber')) {
			if (\t3lib_utility_VersionNumber::convertVersionNumberToInteger(TYPO3_version) >= 6000000) {
	...
	function getImagesDirectory($limitImages = 0) {
		if (is_dir( $this->config['startingpoint'] )) {
			...
			if ($this->config['text'] != '') {
				$caption = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode($this->config['splitRecord'], $this->config['text'] );
			}
			...
			foreach ( $images as $key => $value ) {
				...
				// caption text
				if ($caption[$key]) {
					$text = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode($this->config['splitComment'], $caption[$key] );
				} else {
...
*/
	...
	function addImage($path, $title, $description, $thumb, $lightbox, $uniqueID, $limitImages = 0) {
		...
		$bigImage = str_replace(' ', '_', $this->cObj->IMG_RESOURCE( $imgTSConfigBig ));
		if ($this->isVersion6) {
		...
		if ($this->isVersion6) {
			$lightbox = str_replace( ' ', '_', $lightbox ); // search for empty chars, thx maxhb  
		} else {
		...
		if ($thumb) {
			...
			if ($this->isVersion6) {
				$thumbImage = str_replace(' ', '_', $thumbImage);
			}
				
			#$thumbImage = '<img src="' . $this->cObj->IMG_RESOURCE( $imgTSConfigThumb ) . '" class="thumbnail" />';
			$thumbImage = '<img src="' . $thumbImage . '" class="thumbnail" />';
		}
